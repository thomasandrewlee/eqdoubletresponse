...
if weightFreqByAmp  # perform weighted median and percentiles
    ...
    TXFRstd = map(x->std(log10.(TXFRM[x,gidx[x]])),1:lastindex(TXFRF))
    TXFRstd_smth = movmean(TXFRstd,Nsmth)
    ...
    TXFRD95_smth = movmean(TXFRD95,Nsmth)
    TXFRstd = map(x->std(filter(!isnan,log10.(TXFRM[x,:]))),1:lastindex(TXFRF))
    TXFRstd_smth = movmean(TXFRstd,Nsmth)
end
# plot transfers
...
plot!(hptg,1 ./TXFRF[2:end],log10.(TXFRD_smth[2:end])-TXFRstd_smth[2:end],lc=:lightgray,ls=:dash,label="1-sigma",lw=1)
plot!(hptg,1 ./TXFRF[2:end],log10.(TXFRD_smth[2:end])+TXFRstd_smth[2:end],lc=:lightgray,ls=:dash,label="",lw=1)
...
plot!(hptgl,log10.(1 ./TXFRF[2:end]),log10.(TXFRD_smth[2:end]).-TXFRstd_smth[2:end],lc=:lightgray,ls=:dash,label="1-sigma",lw=1)
plot!(hptgl,log10.(1 ./TXFRF[2:end]),log10.(TXFRD_smth[2:end]).+TXFRstd_smth[2:end],lc=:lightgray,ls=:dash,label="",lw=1)
