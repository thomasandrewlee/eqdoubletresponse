...
windowstart = -Dates.Minute(20)
windowend = Dates.Minute(60) # window for surface waves
...
for i in ProgressBar(1:lastindex(oldEQtme))
    ...
    if sum(.!isnan.(oldD[targidx]))/length(targidx) >= datathresh # data is there
        ...
        hpw = plot(oldT[targidx],oldD[targidx],lc=:black,legend=false,ylabel="pixels",
            title=string(Dates.format(oldEQtme[i],"yyyy-mm-ddTHH:MM:SS.sss"),
                "; M",oldEQmag[i],"; (",oldEQlat[i],", ",oldEQlon[i],
